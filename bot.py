    ...
    if content_type == 'text':
        ...
        if text == '/help':
            ...
            if check_real_email(email, chat_id) and not check_existing_email(email) :
                check_new_id(chat_id, name)   # in case it doesn't exist the method check_new_id will add it to the database
...
def check_new_id(chat_id, chat_name):
    with open("database.txt", "r") as ids:
        data = ids.read()
        for key in data:  # keys are the ids
            if chat_id == key:
                return
...
def add_id(chat_id):
    with open("database.txt", "a") as file:
        ...
        key = chat_id
        data.setdefault(key, [])  # associates the key with a list of values and not only one value and also adds the key (with a corresponding empty list)
...
def add_email(email, chat_id):
    with open("database.txt", "a") as file:
        ...
        for key in data:
            if key == chat_id:  # it will sure find it because if there wasn't already that id it has just been created by add_id
...
def send_email_alert(data, prevdata, data_type):
    ...
    str_send_email += 'NEW VALUE: ' + prevdata + ' PREVIOUS VALUE: ' + data
...
sender_address = 'damybert@gmail.com'
sender_pass = 'damiano2001'
